...
export const generateChatResponse = async (chatHistory, message, apiKey) => {
    ...
    if (!genAI) {
        if (!apiKey) {
            return {
                role: 'assistant',
                content: "🛠️ [Modo Simulación]: No detecto tu API Key en el archivo .env. Asegúrate de añadir VITE_GEMINI_API_KEY y reiniciar el servidor de desarrollo (npm run dev)."
            };
        }
